...
def main():
    ...
    try:
        result = subprocess.run(
            ['python3', 'nwsl-live.py', '--tz', args.tz],
            capture_output=True,
            text=True,
            check=True
        )
    ...
    # FIX: Launch auto_refresh.py with sudo so it can call nwsl-live.py directly
    # (without a nested sudo that would fail due to TTY restrictions).
    print("\n[2/3] Starting auto-refresh service (updates every 15 seconds)...")
    refresh_process = subprocess.Popen(
        ['sudo', 'python3', 'auto_refresh.py', '--tz', args.tz],
    ...
    display_cmd = ['sudo', 'python3', 'run_nwsl_scoreboard.py']
    if args.team:
    ...
    print("Data refreshes every 15 seconds")
    print("Press Ctrl+C to stop")
    print("=" * 60 + "\n")
